...
## Welcome to Niri21!!!! Hope you enjoy the dots!~
## Installer built for various package managers listed below!

readonly DOTS_REPO="https://github.com/Lynndroid21/Niri21"
readonly DOTS_NAME="Niri21"
...
install_N21deps() {
    ...
    case $mgr in
        pacman) yay -S brightnessctl flameshot waybar waypaper swaybg pywal wl-clipboard hyprlock swaync fish kitty rofi wlogout pipewire-pulse batsignal polkit-gnome starship syscgo nmtui bluetui ;;
        xbps) sudo xbps-install -y brightnessctl flameshot waybar waypaper swaybg pywal wl-clipboard hyprlock swaync fish kitty rofi wlogout pipewire-pulse batsignal polkit-gnome starship nmtui bluetui ; curl -fsSL https://raw.githubusercontent.com/Nomadcxx/sysc-Go/master/install.sh | sudo bash ;;
        dnf) sudo dnf install -y brightnessctl flameshot waybar waypaper swaybg pywal wl-clipboard hyprlock swaync fish kitty rofi wlogout pipewire-pulse batsignal polkit-gnome starship nmtui bluetui ; curl -fsSL https://raw.githubusercontent.com/Nomadcxx/sysc-Go/master/install.sh | sudo bash ;;
        zypper) sudo zypper install -y brightnessctl flameshot waybar waypaper swaybg pywal wl-clipboard hyprlock swaync fish kitty rofi wlogout pipewire-pulse batsignal polkit-gnome starship nmtui bluetui ; curl -fsSL https://raw.githubusercontent.com/Nomadcxx/sysc-Go/master/install.sh | sudo bash ;;
        apt) sudo apt update && sudo apt install -y brightnessctl flameshot waybar waypaper swaybg pywal wl-clipboard hyprlock swaync fish kitty rofi wlogout pipewire-pulse batsignal polkit-gnome starship nmtui bluetui ; curl -fsSL https://raw.githubusercontent.com/Nomadcxx/sysc-Go/master/install.sh | sudo bash ;;
...
install_N21dots() {
    ...
    command mkdir $DOTS_INSTALL_DIR/niri
    command cp -r -v -i $PATH_TO_GIT_CLONE/configs/niri $DOTS_INSTALL_DIR
...
ins_N21proc() {
    if $updFin = true; then
       if $ins_N21deps = true; then
            if $ins_N21dots = true; then
...
main() {
    ...
    while true; do
        if command -v gum &>/dev/null; then
            gum style --bold --padding "0 2" --border double --border-foreground 12 "❤ Niri21 Dots Installer ❤"
        else
            echo -e "\e[36m❤ Niri21 Dots Installer ❤\e[0m"
        fi
    sleep 4
    ins_N21proc
    done
