...
DotsUpd() {
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        command cp -r -v -i $DOTS_INSTALL_DIR/niri $DOTS_INSTALL_DIR/backup
...
install_N21dots() {
    ...
    command cp -r -v $PATH_TO_GIT_CLONE/configs/hypr $DOTS_INSTALL_DIR
    ...
    command cp -r -v $PATH_TO_GIT_CLONE/configs/niri $DOTS_INSTALL_DIR
